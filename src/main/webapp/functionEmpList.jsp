...
<%
	...
	/*
	    ...
	    salary, salary/12, round(salary/12, 2),
	    hire_date, EXTRACT(Year from hire_date)
	    from employees;
	*/
	
	sql = "select 번호, 이름, 이름첫글자, 연봉, 급여, 입사날짜, 입사년도 from (select rownum 번호, last_name 이름, substr(last_name, 1, 1) 이름첫글자, salary 연봉, round(salary/12, 2) 급여, hire_date 입사날짜, extract(year from hire_date) 입사년도 from employees) where 번호 between ? and ?";
	...
	System.out.println(rs + "<--- functionEmpList rs");
